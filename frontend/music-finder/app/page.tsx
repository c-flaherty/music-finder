...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#FFF5D1] flex flex-col items-center py-8 md:py-12 px-4">
      {/* Header & Hero */}
      <header className="w-full max-w-2xl mx-auto flex flex-col items-center mb-8 md:mb-12">
        <div className="flex items-center gap-3 mb-4 md:mb-6">
          ...
          <span className="text-xl md:text-2xl font-['Proxima_Nova'] font-extrabold tracking-tight text-[#502D07]">Cannoli</span>
        </div>
        <style jsx global>{proximaNovaExtrabold}</style>
        <h1 className="font-['Proxima_Nova'] text-4xl md:text-5xl lg:text-6xl font-extrabold text-[#F6A23B] text-center mb-3 md:mb-4 tracking-tight leading-tight">
        ...
        <form className="w-full max-w-xl flex items-center bg-white border border-[#DDCDA8] rounded-2xl shadow-md px-4 md:px-5 py-3 focus-within:ring-2 focus-within:ring-[#F6A23B] transition-all mx-4">
          ...
          <input
            type="text"
            className="flex-1 bg-transparent outline-none px-3 py-2 text-base md:text-lg text-[#502D07] placeholder-[#838D5A]"
            placeholder="that song about a roof in New York?"
            value={search}
            onChange={e => setSearch(e.target.value)}
            disabled
          />
          <button className="ml-2 px-4 md:px-5 py-2 bg-[#01D75E] text-white rounded-xl font-semibold shadow hover:bg-[#01c055] active:bg-[#00b04d] transition-colors" disabled>
            Ask
          </button>
        </form>
      </header>
      ...
      <div className="w-full flex justify-center mb-6 md:mb-8 px-4">
        {isAuthenticated ? (
          <button
            ...
            className="flex items-center gap-3 px-6 md:px-7 py-2.5 md:py-3 bg-[#502D07] text-white rounded-xl text-sm md:text-base font-semibold shadow hover:scale-105 active:scale-95 transition-transform w-full md:w-auto justify-center"
          >
          ...
          <a 
            ...
            className="flex items-center gap-3 px-6 md:px-7 py-2.5 md:py-3 bg-[#01D75E] text-white rounded-xl text-sm md:text-base font-semibold shadow hover:scale-105 active:scale-95 transition-transform w-full md:w-auto justify-center"
          >
      ...
      <footer className="w-full flex flex-col items-center gap-3 md:gap-4 pb-2 mt-auto px-4">
        <div className="flex gap-3 md:gap-4 flex-wrap justify-center">
          <span className="text-[#502D07] text-xs">@cannoliworld</span>
          <a href="#" className="text-[#502D07] hover:text-[#F6A23B] text-xs">Twitter</a>
          <a href="#" className="text-[#502D07] hover:text-[#4D41E6] text-xs">ProductHunt</a>
        </div>
        <span className="text-[#838D5A] text-xs">Made with ❤️ in SF</span>
      </footer>
    </div>
  );
}
